...
======*/
resource "aws_cloudwatch_log_group" "prowl" {
  name = "prowl"
...
======*/
resource "aws_ecr_repository" "prowl_app" {
...
data "template_file" "web_task" {
  ...
  vars {
    image           = "${aws_ecr_repository.prowl_app.repository_url}"
    log_group       = "${aws_cloudwatch_log_group.prowl.name}"
  }
}
...
resource "aws_alb" "alb_prowl" {
  name            = "${var.environment}-alb-prowl"
...
resource "aws_alb_listener" "prowl" {
  load_balancer_arn = "${aws_alb.alb_prowl.arn}"
  port              = "80"
  protocol          = "HTTP"
